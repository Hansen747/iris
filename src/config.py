...
DATA_ROOT_DIR = f"{IRIS_ROOT_DIR}/data"
...
CODEQL_DB_PATH = f"{DATA_ROOT_DIR}/codeql-dbs"

# PROJECT_SOURCE_CODE_DIR contains the Java projects. 
PROJECT_SOURCE_CODE_DIR = f"{DATA_ROOT_DIR}/cwe-bench-java/project-sources"

# PACKAGE_MODULES_PATH contains each project's internal modules. 
PACKAGE_MODULES_PATH = f"{DATA_ROOT_DIR}/cwe-bench-java/package-names"

# OUTPUT_DIR is where the results from running Iris are stored.
OUTPUT_DIR = f"{IRIS_ROOT_DIR}/output"

# ALL_METHOD_INFO_DIR  
ALL_METHOD_INFO_DIR = f"{DATA_ROOT_DIR}/cwe-bench-java/data/fix_info.csv"
...
CVES_MAPPED_W_COMMITS_DIR = f"{DATA_ROOT_DIR}/cwe-bench-java/data/project_info.csv"

# Path to cwe-bench-java directory submodule.
CWE_BENCH_JAVA_DIR = f"{DATA_ROOT_DIR}/cwe-bench-java"